...
bot_btn1 = button.Button('Bot ON', 100, 50, (340, 610), 1, screen, font)
bot_btn2 = button.Button('Bot OFF', 100, 50, (340, 670), 1, screen, font)
...
def board():
    ...
    screen.blit(timer, (10, 690))

    #instructions = font.render('Press R to restart', True, 'black')
    #instructions2 = font.render('Press ESC to return to menu', True, 'black')
    #screen.blit(instructions, (10, 730))
    #screen.blit(instructions2, (10, 770))
            ...
            for j in range(6):
                ...
                for q in range(i + 1):
                    if board[5 - q][j] == 0:
                        shift += 1
                if shift > 0:
                    board[4 - i + shift][j] = board[4 - i][j]
                    board[4 - i][j] = 0
                if 5 - i + shift <= 5:
                    if board[4 - i + shift][j] == board[5 - i + shift][j] and not merged[5 - i + shift][j] and not merged[4 - i + shift][j]:
                        board[5 - i + shift][j] *= 2
                        score += board[5 - i + shift][j]
                        board[4 - i + shift][j] = 0
                        merged[5 - i + shift][j] = True
            ...
            for j in range(6):
                ...
                for q in range(j):
                    if board[i][5 - q] == 0:
                        shift += 1
                if shift > 0:
                    board[i][5 - j + shift] = board[i][5 - j]
                    board[i][5 - j] = 0
                if 6 - j + shift <= 5:
                    if board[i][6 - j + shift] == board[i][5 - j + shift] and not merged[i][6 - j + shift] and not merged[i][5 - j + shift]:
                        board[i][6 - j + shift] *= 2
                        score += board[i][5 - j + shift]
                        board[i][5 - j + shift] = 0
                        merged[i][6 - j + shift] = True
    return board
...
while run:
    ...
    seconds = math.trunc((start-end)*(-1))
    bot_btn1.draw()
    bot_btn2.draw()
    if spawn or count < 2:
        board_values, game_over = new_board(board_values)
        spawn = False
        count += 1
    ...
    if bot_btn1.check_click():
        bot = True
        player = False

    if bot_btn2.check_click():
        bot = False
        player = True
